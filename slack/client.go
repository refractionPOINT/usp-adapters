...
func (a *SlackAdapter) fetchEvents(url string) {
...
func (a *SlackAdapter) updateBearerToken() error {
...
func (a *SlackAdapter) makeOneRegistrationRequest(url string) (utils.Dict, error) {
	// Prepare the request.
...
func (a *SlackAdapter) makeOneListRequest(url string) ([]listItem, string) {
	// Prepare the request.
...
func (a *SlackAdapter) makeOneContentRequest(url string) []utils.Dict {
	// Prepare the request.
	req, err := http.NewRequest("GET", url, &bytes.Buffer{})
	if err != nil {
		a.doStop.Set()
