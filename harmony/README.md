...
harmony:
  ...
  #
  # This is the gateway BASE url — the API paths are appended to it. Do not
...
- it refuses the submit with a 403 whose body carries `Unauthorized to perform operations on the given Cloud Service`.

`cloud_services` defaults to the **full** Harmony suite, so a tenant licensed for only part of it will warn about the rest until the list is narrowed.
...
A *filtered* query (non-empty `filter`) is bounded server-side by the predicates, so it scales independently of total mail volume — the gateway's per-query record ceiling is not approached. The **unfiltered firehose preset** (window mode, no `filter`, `include_splits: true`) is the exception: it is bounded only by the received-time window, so on a very high-volume tenant a long `lookback` can hit the ceiling. Keep `lookback` short for that preset (the 1h default is intentional), or use a filtered query.
